...
"
Script plots dtau_s timseries for CMIP5 and CMIP6 ESMs in future SSP and RCP simulations.
"
...
for rcp_option in range(0, rcp_options_length):
    ...
    for model_i in range(0, n_models):
            ...
            rh_historical_cube = select_time(rh_historical_cube, 2000, 2010)
            ...
            # Rh (Future)
            rh_cube = combine_netCDF_cmip5('/home/rmv203/DATA/cmip5_data/rh_Lmon_'+model+'_'+rcp+'_*', 'heterotrophic_respiration_carbon_flux', model)
            rh_cube = open_netCDF(rh_cube)
            rh_cube = select_time(rh_cube, 2005, 2100)
            ...
            # dtau calculations
            historical_tau = soil_carbon_historical_data/rh_historical_data
            future_tau = soil_carbon_future_data/rh_data
            delta_tau_cmip5 = future_tau - historical_tau
            ...
            if model=='CCSM4' or model=='NorESM1-M':
                plt.plot(time_dimension, delta_tau_cmip5, color=model_colors_cmip5[model_i], linestyle='dashed')
            else:
                plt.plot(time_dimension, delta_tau_cmip5, color=model_colors_cmip5[model_i])
...
for ssp_option in range(0, ssp_options_length):
    ...
    for model_i in range(0, n_models):
            ...
            cSoil_historical_cube = open_netCDF(cSoil_historical_cube)
            cSoil_historical_cube = select_time(cSoil_historical_cube, 2000, 2010)
            cSoil_historical_cube = time_average(cSoil_historical_cube)
            cSoil_historical_cube = global_total_percentage(cSoil_historical_cube, landfrac=landfraction, latlon_cons=None)
            cSoil_historical_data = cSoil_historical_cube.data
            # cSoil (Future)
            ...
            if model=='ACCESS-ESM1-5' or model=='BCC-CSM2-MR' or model=='CanESM5' or model=='CNRM-ESM2-1' or model=='IPSL-CM6A-LR' or model=='MIROC-ES2L' or model=='MPI-ESM1-2-LR' or model=='NorESM2-LM' or model=='GFDL-ESM4':
                ...
                cLitter_historical_cube = open_netCDF(cLitter_historical_cube)     
                cLitter_historical_cube = select_time(cLitter_historical_cube, 2000, 2010)   
                cLitter_historical_cube = time_average(cLitter_historical_cube)
                cLitter_historical_cube = global_total_percentage(cLitter_historical_cube, landfrac=landfraction, latlon_cons=None)
                cLitter_historical_data = cLitter_historical_cube.data
                soil_carbon_historical_data = cSoil_historical_data + cLitter_historical_data
            ...
            rh_historical_data = rh_historical_cube.data*86400.*360.
            # Rh (Future)
            rh_cube = combine_netCDF_cmip6('/home/rmv203/DATA/cmip6_data/rh_Lmon_'+model+'_'+ssp+'_*', model)
            rh_cube = open_netCDF(rh_cube)
            ...
            historical_tau = soil_carbon_historical_data/rh_historical_data
            future_tau = soil_carbon_future_data/rh_data
            delta_taus_cmip6 = future_tau - historical_tau
            ...
            if model=='ACCESS-ESM1-5' or model=='CESM2' or model=='MIROC-ES2L' or model=='MPI-ESM1-2-LR' or model=='NorESM2-LM' or model=='UKESM1-0-LL':
                plt.plot(time_dimension, delta_taus_cmip6, color=model_colors[model_i], linestyle='dashed')
            else:
                plt.plot(time_dimension, delta_taus_cmip6, color=model_colors[model_i])
...
#%%
fig_figure1.savefig('figures/dtau_timeseries_cmip5cmip6_v1', bbox_inches='tight')
plt.close()